...
const STATUS_CONFIG = {
    pending:   { label: 'Pending',   className: 'bg-blue-100 text-blue-700',   icon: <Clock size={11} /> },
...
const URGENCY_CONFIG = {
    ...
    routine:   { label: 'Routine',   className: 'bg-blue-50 text-blue-600 border-blue-100', icon: <ClipboardList size={12} /> },
};
...
const SeverityBadge = ({ urgency }) => {
    const configs = {
        ...
        routine:   { label: 'Routine',   className: 'bg-blue-50 text-blue-600 border-blue-100', icon: <CheckCircle size={12} /> }
    };
...
export default function Appointments() {
    ...
    const handleBook = async (e) => {
        ...
        setBooking(true);
        try {
            const date = new Date(`${newAppt.date}T${newAppt.time || '09:00'}`);
            await appointmentService.bookAppointment({ doctor: newAppt.doctor, date, reason: newAppt.reason });
            setShowModal(false);
                ...
                {(user?.role === 'patient' || user?.role === 'admin') && (
                    <div className="flex gap-3">
                        ...
                        <button
                            ...
                            className="bg-blue-600 text-white px-5 py-2.5 rounded-xl hover:bg-blue-700 transition-all shadow-lg flex items-center font-semibold"
                        >
                ...
                {loading ? (
                            ...
                            <div key={appt._id} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-all">
                                <div className="flex flex-col md:flex-row md:items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-4 flex-1">
                                        <div className="w-14 h-14 rounded-2xl bg-blue-50 flex flex-col items-center justify-center shrink-0">
                                ...
                                {appt.aiRecommendation && (
                                    <div className="mt-2 text-[11px] font-bold text-blue-700 bg-blue-50/50 rounded-xl px-4 py-2.5 flex items-center gap-2 border border-blue-100/50">
                                        <Activity size={14} className="text-blue-500" />
                                        <span className="text-blue-400 uppercase tracking-wider">AI Recommendation:</span>
                                        <span className="italic">{appt.aiRecommendation}</span>
                                        {appt.priorityScore > 0 && (
                                            <span className="ml-auto bg-blue-600 text-white px-2 py-0.5 rounded-lg">Priority: {appt.priorityScore}</span>
                                        )}
                                    </div>
                                )}
                                    ...
                                    <div className="mt-4 pt-4 border-t border-gray-50 flex flex-wrap gap-2 items-center">
                                        {canReschedule && (
                                            <button
                                                ...
                                                className="flex items-center gap-1.5 text-sm px-4 py-2 border border-blue-200 text-blue-600 rounded-xl hover:bg-blue-50 font-semibold transition-colors"
                                            >
                    ...
                    <div className="flex flex-col items-center py-16 gap-3 text-center">
                        ...
                        {user?.role === 'patient' && activeTab === 'upcoming' && (
                            <button onClick={() => setShowModal(true)} className="text-blue-600 font-semibold text-sm hover:underline">
                        ...
                        <form onSubmit={handleBook} className="space-y-5">
                                                ...
                                                >
                                                    ...
                                                    <div className="shrink-0 relative">
                                                        {doc.user?.image || doc.image ? (
                                                            ...
                                                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-lg font-black ${
                                                                isSelected ? 'bg-blue-600 text-white' : 'bg-gradient-to-br from-blue-100 to-indigo-200 text-blue-700'
                                                            }`}>
                                                                {initials}
                                                            </div>
                                                        )}
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <p className={`text-xs font-semibold mt-0.5 ${
                                                            isSelected ? 'text-blue-600' : 'text-indigo-500'
                                                        }`}>
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <button
                                    type="submit"
                                    disabled={booking}
                                    className="flex-1 px-4 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-bold disabled:opacity-60 flex items-center justify-center gap-2"
                                >
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl">
                        ...
                        <form onSubmit={handleReschedule} className="space-y-5">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button type="submit"
                                    className="flex-1 px-4 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-bold">
                                    Confirm
                                </button>
                            </div>
                        </form>
                        ...
                        <div className="space-y-4">
                            <div>
                                <h4 className="text-sm font-bold text-gray-700 flex items-center gap-2 mb-2">
                                    <ClipboardList size={16} className="text-blue-500" /> Medical History
                                </h4>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-md flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-8 text-white">
                            <div>
                                <h3 className="text-2xl font-black">AI Voice Booking</h3>
                                <p className="text-indigo-100 text-sm font-medium">Automatic clinical transcription</p>
                            </div>
                            <button onClick={() => { setShowVoiceModal(false); setVoiceResult(null); setError(''); }} className="p-2 hover:bg-white/10 rounded-full transition-colors">
                                <X size={24} />
                            </button>
                        </div>

                        {error && (
                            <div className="mb-6 p-4 bg-red-50 border border-red-100 rounded-2xl text-red-700 text-sm flex items-center gap-3 animate-shake">
                        ...
                        {!voiceResult ? (
                            <div className="flex flex-col items-center py-6 text-center">
                                {/* Language Selection */}
                                <div className="flex bg-gray-100 p-1 rounded-2xl mb-10 w-full max-w-[240px]">
                                    <button 
                                        onClick={() => setSelectedLang('en-US')}
                                        className={`flex-1 py-2 px-4 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 ${selectedLang === 'en-US' ? 'bg-white shadow-md text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                                    >
                                        <Globe size={11} /> ENGLISH
                                    </button>
                                    <button 
                                        onClick={() => setSelectedLang('ta-IN')}
                                        className={`flex-1 py-2 px-4 rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 ${selectedLang === 'ta-IN' ? 'bg-white shadow-md text-blue-600 font-tamil' : 'text-gray-500 hover:text-gray-700 font-tamil'}`}
                                    >
                                        <Languages size={11} /> தமிழ்
                                    </button>
                                </div>
                                    ...
                                    >
                                        {voiceLoading ? (
                                            ...
                                            <>
                                                ...
                                                <span className="text-[10px] text-white/80 font-black mt-2 uppercase tracking-widest leading-none">
                                ...
                                <div className="space-y-2">
                                    <p className="text-lg font-black text-gray-900">
                                        {isRecording ? `${Math.floor(recordingTime / 60)}:${(recordingTime % 60).toString().padStart(2, '0')}` : 'Ready to record'}
                                    </p>
                                    <p className="text-sm text-gray-400 max-w-[260px]">
                                        {isRecording 
                                            ? "Recording your booking details. Speak clearly into the microphone." 
                                            : "Hold to record your name, date, and reason in your preferred language."}
                                    </p>
                                </div>
                            </div>
                        ) : (
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100">
                                            <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-1">Doctor</p>
                                            <p className="font-bold text-blue-900 truncate">{voiceResult.appointment?.doctor?.name || 'Assigned'}</p>
                                        </div>
