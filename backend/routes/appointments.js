    ...
    try {
        ...
        if (req.user.role === 'doctor') {
            ...
            appointments = appointments.map(appt => {
                ...
                if (!patientId) return appt;
                
                const profile = clinicalProfiles.find(cp => cp.user.toString() === patientId.toString());
                if (profile) {
...
router.post('/voice-booking', auth(['patient']), upload.single('audio'), async (req, res) => {
    try {
        ...
        } else if (parsed_data.date_hint === 'next week') {
        ...
        if (parsed_data.time_hint) {
            const timeMatch = parsed_data.time_hint.match(/(\d{1,2}):(\d{2})(?:\s*(am|pm|o'clock))?/i);
            if (timeMatch) {
